...
var StratumClient = function(options){
    ...
    function handleAuthorize(message, replyToSocket){
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            /*update Oct 10: setting static dif */
            if(_this.workerPass != "x"){
                set_dif = Number(_this.workerPass);
                if(!isNaN(set_dif)){
                    console.log(options.socket.localPort);
                    Object.keys(options).forEach(function(o){
                        console.log(o);
                    });    
                        
                    is_vardiff = options.ports[options.socket.localPort]["varDiff"];
                    if(!is_vardiff){
                        min_dif_allowed = options.ports[options.socket.localPort]["diff"];
                        if(set_dif<min_dif_allowed){
                            set_dif = min_dif_allowed;
                        }
                        if(String(options.socket.localPort)=="3075"){
                            if(set_dif<1000000){
                                set_dif = 1000000;
                            }
                        }
                        pendingDifficulty = set_dif;
                    _this.pendingDifficulty = set_dif;
                    }
                }
            }
            /*end */
