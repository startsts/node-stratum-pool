...
var pool = module.exports = function pool(options, authorizeFn){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            */
            if (!isValidBlock)
                ...
                emitLog('DEBUG SUBMIT BLOCK ' + String(shareData.blockHash));
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            if (rpcResults.submitblock.message === 'Method not found' || options.forbidsubmitblock){
        ...
        _this.stratumServer.on('started', function(){
            ...
            client.on('difficultyChanged', function(diff){
                ...
                GetBlockTemplate(function(error, rpcData, processedBlock){
                    ...
                    if(client.shares){
                        if(client.shares.valid){
                            if(client.shares.valid>=2){
                                rebroadcast = false;
                            }else{
                                client.shares.valid += 1;
                            }
                        }else{
                            client.shares.valid = 1;
                        }
                    }
                    if(rebroadcast)_this.jobManager.updateCurrentJob(rpcData);
                });
                /*end */
